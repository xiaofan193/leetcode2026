...
| 📐 数组 | 双指针 / 原地修改 / 排序后处理 / 堆取前 k 小 | [array](./array) |
...
数组题围绕**下标的移动与覆盖**展开，常见套路是**双指针（快慢 / 左右 / 荷兰国旗）、原地修改、先排序再复用有序性**，目标多是 O(1) 额外空间；一旦答案要求「第 k 小」这类有序输出，就改用**小顶堆做多路归并**，只算到第 k 个为止。

对应目录：[`array/`](./array)，内含 `golang/`、`python/`、`ts/` 三个语言版本（下表收录 14 题，均三语齐备）。
...
题面已经把下标关系写死（1920 的 `ans[i] = nums[nums[i]]`），照着公式一次遍历即可，难点只在边界和返回值这些零碎细节上。

| 题号 | 题目 | Go | Python | TS |
| ---- | ---- | :-: | :-: | :-: |
| 1920 | 基于排列构建数组 | [✅](./array/golang/build_array_from_permutation.go) | [✅](./array/python/build_array_from_permutation.py) | [✅](./array/ts/build_array_from_permutation.ts) |

> 💡 **1920** 的 `nums` 是 0 到 `n-1` 的排列，所以 `nums[i]` 本身就是合法下标，不需要额外判界；三个版本都是「先按长度分配结果数组，再逐位赋值」，注意循环必须覆盖到最后一个下标（写成 `len(nums) - 1` 会漏掉末尾元素）。
...
答案要的是「第 k 小」而不是「排完序再取第 k 个」时，不必把全部组合都算出来：把候选组织成若干条**递增链**，用小顶堆每次弹出当前最小值，弹一个就沿它所在的链推进一格，代价只跟 k 相关。

| 题号 | 题目 | Go | Python | TS |
| ---- | ---- | :-: | :-: | :-: |
| 373 | 查找和最小的 K 对数字 | [✅](./array/golang/find_k_pairs_with_smallest_sums.go) | [✅](./array/python/find_k_pairs_with_smallest_sums.py) | [✅](./array/ts/find_k_pairs_with_smallest_sums.ts) |
| 1439 | 有序矩阵中的第 k 个最小数组和 | [✅](./array/golang/array_1439.go) | [✅](./array/python/array_1439.py) | [✅](./array/ts/array_1439.ts) |

> 💡 **373** 是这套模板的原型：把 `(i, j)` 看成行列都递增的矩阵，初始只有 `(0,0)`，弹出堆顶后推进右邻居和下邻居，并**只让第一列向下扩展**，避免同一个格子被两条路径重复入堆。**1439** 直接复用它——把「每行选 1 个」拆成两两合并，每轮只保留前 k 小的和再与下一行合并；每轮截断到 k 是安全的，因为排不进前 k 的中间和，后续无论怎么选都凑不出更小的完整和。
>
> 📎 1439 另附 Go 单元测试（含随机对拍暴力解）：[测试](./array/golang/array_1439_test.go)
